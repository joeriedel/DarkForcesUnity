...
namespace CSharpSynth.Synthesis
{
    public class StreamSynthesizer
    {
		...
		private double[] slowTunePositions_;
        ...
        public void setPitchBend(int channel, float semitones)
        {
            if (channel > -1 && channel < tunePositions_.Length && semitones >= -12.00f && semitones <= 12.00f)
            {
				...
				slowTunePositions_[channel] = semitones;
            }
        }
		public void setAllChannelPitchBend(float semitones) {
			for (int i = 0; i < tunePositions_.Length; ++i) {
				setPitchBend(i, semitones);
			}
		}
		public void setAllChannelSlowPitchBend(float semitones) {
			for (int i = 0; i < tunePositions_.Length; ++i) {
				slowTunePositions_[i] = semitones;
			}
		}
        ...
        {
			//Debug.Log("Sample Time: " + seq.SampleTime + ", Event: " + seq.EventIndex);
            ...
            if (seq != null && seq.isPlaying)//Use sequencer
            {
                ...
                for (int x = 0; x < seqEvent.Events.Count; x++)
                {
                    ...
                    if (waitTime != 0)
                    {
                        ...
                        while (node != null)
                        {
                            if (oldtime < 0 || waitTime < 0)
                                throw new Exception("Sample time out of sync");
					...
					if (seq.ProcessMidiEvent(seqEvent.Events[x])) {
						// restart seq
						seqEvent = seq.Process(samplesperBuffer - oldtime);
						if (seqEvent == null) {
							break;
						}
						oldtime = 0;
